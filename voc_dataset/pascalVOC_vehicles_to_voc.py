...
labels_want = ["car", "truck", "bus", "motorbike", "bicycle"]
pose_label = False
...
target_voc_path = "/DATA1/Datasets_mine/labeled/vehicles/voc"
...
def getLabels(imgFile, xmlFile):
    ...
    for object in objects:
        ...
        for id, elem in enumerate(tmpArrays):
            ...
            for id, elem in enumerate(tmpArrays):
                if(id in id_list):
                    num = int(float(elem.firstChild.data))
                    if(num<0): num = 0
                    labelXmin.append(num)
